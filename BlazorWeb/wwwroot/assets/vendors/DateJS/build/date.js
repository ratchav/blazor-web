 ...
 */(function () {
	...
	var getText = {
		getFromKey: function (key, countryCode) {
			...
			if (key.charAt(0) === "file:///") { // Assume it's a regex
		...
		buildFromRegex: function (key, countryCode) {
			...
			if (Date.CultureStrings && Date.CultureStrings[countryCode] && Date.CultureStrings[countryCode][key]) {
				...
				output = new RegExp(key.replace(new RegExp("file:///", "g"),""), "i");
			}
			return output;
		}
	};
...
(function () {
	...
	$P.Numeric = {
		...
		parse: function (s) {
			...
			if (s.length < 5 && s.indexOf(".") < 0 && s.indexOf("file:///") < 0) { // assume it's just a year.
				time.year = s;
				return $P.processTimeObject(time);
			}
